...
Script which plots 
1. topographic plots of all channels with REST and MOT condition 
data for all subjects, 
2. ROI vector PSD information for all subjects. 

The plotting range can be set as desired (sometimes visualization of 
                                          alpha and beta is useful)

Plots are saved in the folder specified by path_to_figs.
...
fmin_plot=13
fmax_plot=30
side = 'left' # choose hemisphere, options 'left' and 'right'
...
for p in range(0, len(df_meta)): 
    ...
    for h in range(0, len(txtfiles)-n_exceptions):  
        ...
        if subj==pat_id:
            ...
            ftitle=('topoplot/topo_subject_%s_%s-%sHz.pdf' % (subj, fmin_plot, fmax_plot)) 
            ...
            plt.legend()
            ftitle=('vector_PSD/perdiodic_indiv_right_fft%s_%s_%s.png' % (n_fft, subj, side)) 
            fname  = path_to_figs + ftitle
            plt.gcf().savefig(fname)    
